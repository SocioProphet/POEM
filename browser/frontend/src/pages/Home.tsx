...
const instrumentCards: instrumentProps[] = [
  {
    ...
    description: "Standardized tools for measuring depression",
    count: 100,
    Icon: Frown,
  },
  {
    name: "Anxiety instruments",
    description: "Standardized tools for measuring anxiety",
    count: 100,
    Icon: ClipboardCheck,
  },
]; 
  ...
  return (
    ...
    focus-within:border-amber-400 flex justify-between"
          ...
          {results &&
            results.map((results, index) => (
              <div
                ...
                className="text-left bg-white max-w-xl min-w-lg p-2 border border-amber-400 transition-all duration-200 hover:border-2 hover:bg-gray-200"
                key={index}
              >
                {results.title}
              </div>
        ...
        <div className="flex justify-center mt-12 text-xl gap-4">
          <Link
            to="/instruments"
            className="px-3 py-2 gap-2 flex transition-all duration-200 bg-amber-500 hover:bg-amber-600 shadow-xl hover:shadow-2xl justify-between items-center"
          >
            Discover Instruments
            <BookOpen />
          </Link>
          <Link
            className="p-3 border border-white transition-all duration-200 hover:bg-slate-600"
            to="/Glossary"
          >
            Learn More
          </Link>
